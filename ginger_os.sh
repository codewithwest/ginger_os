...
run_step "13_phase3_system" "sudo chroot \"$LFS\" /tools/bin/bash -c 'HOME=/root TERM=\"$TERM\" MAKEFLAGS=\"-j$(nproc)\" PATH=/bin:/usr/bin:/sbin:/usr/sbin:/tools/bin bash scripts/phases/build-phase3.sh'"


# 14. Kernel
run_step "14_kernel" "sudo chroot \"$LFS\" /bin/bash -c 'HOME=/root TERM=\"$TERM\" MAKEFLAGS=\"-j$(nproc)\" PATH=/bin:/usr/bin:/sbin:/usr/sbin bash scripts/phase4-boot/01-kernel.sh'"
