...
class GingerTUI:
    ...
    def render_header(self):
        ...
        if self.executing_step is not None:
            ...
            if self.engine.overall_start_time:
                ...
                stat_line.append(
                    f" SYS_UPTIME: {self.format_time(overall_elapsed)}",
                    style="bold bright_blue",
                )
                stat_line.append(" | ", style="dim")

            stat_line.append(
                f"PHASE_RUN: {self.format_time(phase_elapsed)}",
            ...
            if self.engine.current_pkg:
                pkg_line = Text()
                pkg_line.append(
                    f" TARGET: {self.engine.current_pkg}", style="bright_yellow"
                )
                ...
                if self.engine.pkg_start_time:
                    ...
                    pkg_line.append(
                        f" [⏱ {self.format_time(pkg_elapsed)}]", style=self.theme_color
                    )

                if self.engine.total_pkg_count > 0:
                    pkg_line.append(
                        f" ({self.engine.current_pkg_idx}/{self.engine.total_pkg_count})",
                        style="dim",
                    )
            ...
            if self.engine.overall_start_time:
                overall_elapsed = time.time() - self.engine.overall_start_time
                idle_line.append(
                    f" [TOTAL_RUNTIME: {self.format_time(overall_elapsed)}]",
            ...
            metrics.append("\n\n")
            metrics.append(
                "Waiting for sequence binary initiation sequence...\n\n",
                style="dim italic",
            )
            ...
            if self.auto_all:
                metrics.append(
                    "Autonomous sequence engaged. Standing by for synchronization.",
                    style="italic dim bright_magenta",
                )
            else:
                metrics.append(
                    "Awaiting operator 'EXECUTE' directive.",
                    style="italic dim bright_magenta",
                )
    ...
    def render_steps(self):
        ...
        for idx, step in enumerate(self.engine.steps):
            ...
            table.add_row(slot_txt, step.name, state, style=row_style)
    ...
    def render_dashboard(self):
        ...
        if self.executing_step is not None:
            ...
            # Sub-module progress
            if self.engine.current_pkg:
                stats.append("» TARGET: ", style="bright_yellow")
                ...
                if self.engine.pkg_start_time:
                    ...
                    stats.append("  ETR_PKG: ", style="dim")
                    stats.append(
                        f"{self.format_time(p_elapsed)}\n", style=self.theme_color
                    )
    ...
    def render_ai_copilot(self):
        ...
        if self.executing_step is not None:
            ...
            if "host" in step.name.lower():
                thoughts.append(
                    "Verifying ecosystem dependencies. Host environment identified as stable."
                )
            elif "toolchain" in step.name.lower():
                thoughts.append(
                    "Synthesizing binary primitives. Mitigating entropy in the cross-compiler."
                )
            elif "kernel" in step.name.lower():
                thoughts.append(
                    "Orchestrating the heart of GingerOS. Calibrating scheduler and memory safety."
                )
            else:
                thoughts.append(
                    f"Executing directive: {step.name}. Monitoring for syscall anomalies."
                )
        else:
            if self.auto_all:
                thoughts.append(
                    "Autonomous sequence engaged. Standing by for multi-phase synchronization."
                )
            else:
                thoughts.append(
                    "Awaiting operator 'EXECUTE' command. Ready to initiate sequence pulse."
                )
    ...
    def render_matrix(self):
        ...
        matrix.append("\n 💿 STORAGE\n", style="bold bright_white")
        matrix.append(
            f"  Host Disk: [{mini_bar(host_disk)}] {host_disk:.0f}%\n",
            style="bright_blue" if host_disk < 90 else "bright_red",
        )
        matrix.append(
            f"  LFS Disk: [{mini_bar(lfs_disk)}] {lfs_disk:.0f}%\n",
            style=self.theme_color,
        )
    ...
    def render_terminal(self):
        ...
        if self.engine.logs:
            ...
            if self.log_scroll > 0:
                log_content.append(
                    f" ↓ {self.log_scroll} lines from bottom (PgDn to resume)",
                    style="dim yellow",
                )
        else:
            log_content.append("\n [ TERMINAL_STANDBY ]\n", style="bold dim cyan")
    ...
    def update_display(self, layout):
        ...
        if self.show_help:
            ...
            layout["matrix"].update(
                Panel("", border_style=self.theme_color, box=box.ROUNDED)
            )
            layout["terminal"].update(
                Panel("", border_style=self.theme_color, box=box.SQUARE)
            )
        else:
                ...
                while self.running:
                    ...
                    if self.auto_all and self.executing_step is None:
                        ...
                        if self.last_auto_step is not None:
                            ...
                            if last_step.status == "failed":
                                ...
                                self.last_auto_step = None
                                self.engine.log(
                                    f"Auto-Run: Stopped (Step '{last_step.name}' failed)",
                                    "bold red",
                                )
                        ...
                        if next_step_idx != -1:
                            ...
                            self.last_auto_step = None
                            self.engine.log(
                                "Auto-Run: All pending steps finished! 🏁", "bold green"
                            )
                    ...
                    if select.select([sys.stdin], [], [], 0.05)[0]:
                        ...
                        if action == "delete":
                            ...
                            self.engine.package_stepping = (
                                not self.engine.package_stepping
                            )
                        elif action == "resume":
                            self.engine.resume_package()
                            ...
                            threading.Thread(
                                target=lambda: self.engine.take_snapshot(
                                    f"manual_{step.id}"
                                ),
                                daemon=True,
                            ).start()
                            ...
                            if snaps:
                                ...
                                self.engine.log(
                                    "RESTORE: No snapshots found.", "yellow"
                                )
...
def main():
    ...
    parser.add_argument(
        ...
        "--dry-run",
        action="store_true",
        help="Preview build without executing commands",
    )
    parser.add_argument(
        "--no-web", action="store_true", help="Disable the remote monitoring Web UI"
    )
    parser.add_argument(
        "--host", type=str, default="127.0.0.1", help="Web UI host (default: 127.0.0.1)"
    )
    parser.add_argument(
        "-p", "--port", type=int, default=8087, help="Web UI port (default: 8087)"
    )
