...
def create_layout() -> Layout:
    ...
    layout["side"].split_column(Layout(name="roadmap"))

    layout["body"].split_column(Layout(name="status", size=5), Layout(name="logs"))
...
def update_ui(layout: Layout, engine):
    ...
    # Roadmap
    roadmap_table = Table(
        show_header=True, header_style="bold bright_magenta", expand=True, box=None
    )
    ...
    layout["side"]["roadmap"].update(
        Panel(
            roadmap_table,
            title="[bold bright_blue]Roadmap[/bold bright_blue]",
    ...
    if engine.current_step_idx < len(engine.steps):
        ...
        overall_time = (
            time.time() - engine.overall_start_time if engine.overall_start_time else 0
        )
        overall_bar = "█" * int(overall_progress / 2.5) + "░" * (
            40 - int(overall_progress / 2.5)
        )
        status_table.add_row(
            f"[bold bright_cyan]OVERALL:[/bold bright_cyan] [{LASER_GREEN}]{overall_bar}[/] {overall_progress:.0f}%  [bold bright_magenta]⏱ {format_time(overall_time)}[/bold bright_magenta]"
        )

        phase_time = (
            time.time() - engine.phase_start_time if engine.phase_start_time else 0
        )
        status_table.add_row(
            f"[bold bright_cyan]PHASE  :[/bold bright_cyan] {current_step.name} ({current_step.phase}) [bold bright_magenta]⏱ {format_time(phase_time)}[/bold bright_magenta]"
        )
        ...
        if engine.paused_for_error:
            pkg_display = (
                f"[bold bright_red blink]FAILED: {pkg_display}[/bold bright_red blink]"
            )
        status_table.add_row(
            f"[bold bright_yellow]PACKAGE:[/bold bright_yellow] {pkg_display} [bold bright_magenta]⏱ {format_time(pkg_time)}[/bold bright_magenta]"
        )
        ...
        host_bar = "█" * int(engine.storage_stats["host"] / 5) + "░" * (
            20 - int(engine.storage_stats["host"] / 5)
        )
        lfs_bar = "█" * int(engine.storage_stats["lfs"] / 5) + "░" * (
            20 - int(engine.storage_stats["lfs"] / 5)
        )

        storage_row = Columns(
            [
                Text.from_markup(
                    f"[bold cyan]STORAGE (Host):[/][{host_color}]{host_bar}[/] {engine.storage_stats['host']:.0f}% "
                ),
                Text.from_markup(
                    f"[bold cyan](LFS):[/][{lfs_color}]{lfs_bar}[/] {engine.storage_stats['lfs']:.0f}%"
                ),
            ]
        )
        ...
        layout["status"].update(
            Panel(
                status_table,
                title="[bold bright_blue]System Status[/bold bright_blue]",
                border_style="bold bright_blue",
            )
        )
    else:
        layout["status"].update(
            Panel(
                Align.center("[bold bright_green]BUILD COMPLETE[/bold bright_green]"),
                title="[bold bright_blue]System Status[/bold bright_blue]",
                border_style="bold bright_blue",
            )
        )
    ...
    if engine.paused_for_error:
        ...
        footer_msg = (
            f"CRITICAL ERROR: {engine.error_msg}"
            if engine.error_msg
            else "🎉 BUILD COMPLETED SUCCESSFULLY"
        )
        footer_text.append(
            footer_msg, style="bold red" if engine.error_msg else "bold green"
        )
    else:
    ...
    layout["footer"].update(
        Panel(Align.center(footer_text), border_style="bold bright_blue")
    )
